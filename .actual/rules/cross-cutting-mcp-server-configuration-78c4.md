# Use Map-Based In-Memory Caching for MCP Server Configuration State: Mcp Server Configuration
...
- **R-CACHE-001** MUST: MCP server configuration state MUST be cached in-memory using Map data structures keyed by server name (for stdio servers) or URL (for SSE/HTTP servers).
...
# Verify .set() operations are used to populate cache
...
<enforcement>
Clause Code MUST NOT skip or defer verification of R-CACHE-001 compliance in configuration management modules.
</enforcement>