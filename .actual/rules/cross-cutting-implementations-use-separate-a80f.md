# Standardize Map-Based In-Memory Cache for Server Configuration State: Implementations Use Separate
...
- **R-CACHE-001** MAY: Implementations MAY use separate Map instances for different server transport types (stdio, SSE, SHTTP) to enforce type-specific indexing strategies.
...
- **R-CACHE-005** SHOULD: Add input validation in readVSCodeSettings and propagateMcpToOpenHands to verify that server objects contain required key fields before Map.set() operations. Consider TypeScript type guards.
- **R-CACHE-006** SHOULD: Monitor memory usage in production environments. If server registries exceed 1000 entries, consider pagination or lazy-loading strategies for configuration reads.
...
<enforcement>
Clause Code MUST NOT skip or defer verification. Code review checklist items, static analysis via grep, and unit test coverage are mandatory before accepting implementations.
</enforcement>