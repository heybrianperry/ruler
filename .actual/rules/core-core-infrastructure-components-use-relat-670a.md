# Core Infrastructure Depends on Shared Agent Abstractions and Configuration Loader: Core Infrastructure Components Use Relative Path
...
- **R-CORE-001** MUST: Core infrastructure components MUST use relative path imports to reference shared internal modules from the agents and core packages.
...
<enforcement>
Claude Code MUST NOT skip or defer verification. All core infrastructure components must be audited for compliance with R-CORE-001 before merge. Violations detected in code review or CI must be rejected or tracked as technical debt requiring refactoring.
</enforcement>