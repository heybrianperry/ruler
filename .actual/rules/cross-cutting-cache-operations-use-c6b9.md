...
- **R-CACHE-001** MUST: Cache operations MUST use Map.set() to insert or update server entries, ensuring idempotent upsert semantics.
- **R-CACHE-002** MUST: Initialize separate Map instances for each server transport type (stdio, SSE, SHTTP) to enforce type-specific indexing strategies.
...
- **R-CACHE-004** MUST: Use Map.set(key, value) for all server upsert operations to ensure idempotent semantics. Avoid manual existence checks followed by insertion.
...
- **R-CACHE-006** SHOULD: Add input validation in readVSCodeSettings and propagateMcpToOpenHands to verify that server objects contain required key fields before Map.set() operations. Consider TypeScript type guards.

### Verify

```bash
# Verify Map.set() usage with server.name or url as keys
