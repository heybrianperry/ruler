# Adopt Async File System Operations as Standard I/O Pattern: File Operation Functions
...
- **R-ASYNC-001** SHOULD: File operation functions SHOULD expose async interfaces (returning Promises) to enable concurrent execution and proper error propagation.
...
<enforcement>
Clause Code MUST NOT skip or defer verification. All new file I/O code must expose async interfaces. Synchronous operations outside approved exceptions (module initialization, CLI startup) are flagged for review. Static analysis rules detect violations; CI pipeline fails if thresholds are exceeded.
</enforcement>