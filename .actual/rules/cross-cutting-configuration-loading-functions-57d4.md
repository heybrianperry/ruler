...
- **R-CONFIG-002** MUST: Configuration loading functions MUST return graceful defaults or null values when parsing fails rather than propagating exceptions.
...
- **R-CONFIG-004** MUST: Use Zod's .optional() modifier for non-required configuration fields to support partial configurations.
- **R-CONFIG-005** MUST: Include module-specific prefixes in log messages (e.g., '[ruler]') to facilitate log filtering and debugging.
- **R-CONFIG-006** SHOULD: Validate configuration once during loading and cache validated results to avoid re-parsing on every access.
...
- Configuration loading functions return null or default objects on parse failure
- All configuration schemas include appropriate Zod type definitions
- Non-required configuration fields use .optional() modifier
- Log messages include module-specific prefixes for filtering

<enforcement>
Claude Code MUST NOT skip or defer verification. All rules in this file are mandatory for configuration and agent loading modules. Pull requests with unwrapped JSON.parse calls, missing Zod validation, or exceptions from configuration parsing are rejected. Runtime exceptions from configuration parsing are treated as P1 bugs.
</enforcement>