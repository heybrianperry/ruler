# Standardize applyRulerConfig as Configuration Application Pattern: Configuration Application Perform
...
- **R-CONFIG-001** MUST: Configuration application MUST perform input validation using appropriate parsers (JSON.parse, parseTOML) before applying configuration changes.
...
- **R-CONFIG-003** MUST: Agent implementations MUST import from './IAgent' or './AgentsMdAgent' to maintain interface contract compliance.
- **R-CONFIG-004** MUST: Configuration parsing operations (JSON.parse, parseTOML) MUST be wrapped in try-catch blocks with meaningful error messages.
