# Standardize Agent Public Contract Exports via IAgent Interface: Agent Modules Declare
...
- **R-AGENT-001** MUST: Agent modules MUST declare their primary export as the `api.public.contracts` facet to enable automated contract discovery and validation.
...
- **R-AGENT-003** MUST: Core utility modules (`config-utils.ts`, `capabilities.ts`, `agent-utils.ts`) MUST import and depend on the `IAgent` interface rather than concrete agent classes.
- **R-AGENT-004** MUST: All agent implementations that parse external configuration MUST use standard validation patterns (`JSON.parse`, `parseTOML`) with error handling wrapped in try-catch blocks.
...
- **R-AGENT-007** SHOULD: Use `AbstractAgent` or `AgentsMdAgent` base classes when agents share common functionality, ensuring the base class itself implements `IAgent`.

### Verify

```bash
# Count agent implementations that declare IAgent compliance
grep -r "implements.*IAgent\|extends.*AbstractAgent\|extends.*AgentsMdAgent" src/agents/*.ts | wc -l

# Count agent modules with api.public.contracts export
