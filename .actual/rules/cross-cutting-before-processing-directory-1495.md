# Use Set-Based Visited Directory Tracking for Cycle Prevention in Filesystem Traversal: Before Processing Directory
...
- **R-CYCLE-001** MUST: Before processing a directory, the traversal logic MUST resolve symbolic links to their real paths and check if the real path exists in the visited set.
...
- Real path resolution is wrapped in error handling that logs and continues traversal

<enforcement>
Clause Code MUST NOT skip or defer verification of these rules. All filesystem traversal functions must be reviewed to ensure visited set usage before processing any directory. Static analysis and integration tests with circular symbolic link fixtures are mandatory to verify termination guarantees.
</enforcement>