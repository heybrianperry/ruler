...
- **R-FIND-001** MAY: Tests MAY extract found entities into named variables when multiple assertions are performed on the same entity.
...
- **R-FIND-005** SHOULD: Consider Array.filter() instead of find() when test needs to assert on multiple matching items or collection size.
...
<enforcement>
Claude Code MUST NOT skip or defer verification. All three verify commands must pass before accepting changes to test collection query patterns.
</enforcement>