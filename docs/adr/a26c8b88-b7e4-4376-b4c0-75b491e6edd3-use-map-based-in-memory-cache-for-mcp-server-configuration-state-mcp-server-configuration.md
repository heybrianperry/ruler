...
1. MUST: MCP server configuration state MUST be cached in-memory using Map data structures keyed by server name for STDIO transport or URL for SSE/SHTTP transports

## Policy Block

- MUST MCP server configuration state MUST be cached in-memory using Map data structures keyed by server name for STDIO transport or URL for SSE/SHTTP transports
