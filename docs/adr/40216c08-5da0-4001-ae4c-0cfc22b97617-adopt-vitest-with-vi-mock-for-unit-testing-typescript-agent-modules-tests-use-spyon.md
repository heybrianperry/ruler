# Adopt Vitest with vi.mock() for Unit Testing TypeScript Agent Modules: Tests Use Spyon
...
1. MAY: Tests MAY use vi.spyOn() for partial mocking when only specific methods need to be intercepted

## Policy Block

- MAY Tests MAY use vi.spyOn() for partial mocking when only specific methods need to be intercepted
