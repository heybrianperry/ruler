...
1. SHOULD: File system operations that may block the event loop SHOULD use asynchronous APIs rather than synchronous variants except during application initialization or CLI command execution where blocking is acceptable

## Policy Block

- SHOULD File system operations that may block the event loop SHOULD use asynchronous APIs rather than synchronous variants except during application initialization or CLI command execution where blocking is acceptable
