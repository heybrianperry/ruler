...
1. MUST: All TOML content read from the file system MUST be parsed using @iarna/toml parseTOML() with error handling that catches parsing exceptions

## Policy Block

- MUST All TOML content read from the file system MUST be parsed using @iarna/toml parseTOML() with error handling that catches parsing exceptions
