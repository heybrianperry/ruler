...
1. MUST: JSON.parse operations on external file content MUST be wrapped in try-catch blocks to handle malformed JSON gracefully

## Policy Block

- MUST JSON.parse operations on external file content MUST be wrapped in try-catch blocks to handle malformed JSON gracefully
