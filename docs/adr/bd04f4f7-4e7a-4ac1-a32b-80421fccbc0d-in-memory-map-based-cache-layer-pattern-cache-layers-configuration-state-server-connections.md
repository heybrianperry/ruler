...
1. MUST: Cache layers for configuration state, server connections, and resolved selections MUST use native Map instances as the primary in-memory storage mechanism

## Policy Block

- MUST Cache layers for configuration state, server connections, and resolved selections MUST use native Map instances as the primary in-memory storage mechanism
