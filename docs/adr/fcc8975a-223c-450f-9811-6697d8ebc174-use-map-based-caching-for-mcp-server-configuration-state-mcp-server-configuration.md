...
1. MUST: All MCP server configuration state MUST be cached in Map data structures keyed by unique identifiers (server name for stdio, URL for SSE/SHTTP)

## Policy Block

- MUST All MCP server configuration state MUST be cached in Map data structures keyed by unique identifiers (server name for stdio, URL for SSE/SHTTP)
