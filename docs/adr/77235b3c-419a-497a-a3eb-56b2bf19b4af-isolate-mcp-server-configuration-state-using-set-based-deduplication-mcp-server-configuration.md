...
1. SHOULD: MCP server configuration state SHOULD be isolated per directory level with explicit inheritance rules rather than global shared state

## Policy Block

- SHOULD MCP server configuration state SHOULD be isolated per directory level with explicit inheritance rules rather than global shared state
