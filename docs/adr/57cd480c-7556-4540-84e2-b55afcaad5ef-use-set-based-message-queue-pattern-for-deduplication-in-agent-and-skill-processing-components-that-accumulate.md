...
1. MUST: Components that accumulate items requiring uniqueness MUST use Set data structures (e.g., seen.add(key), targets.add(target)) rather than arrays with manual deduplication

## Policy Block

- MUST Components that accumulate items requiring uniqueness MUST use Set data structures (e.g., seen.add(key), targets.add(target)) rather than arrays with manual deduplication
