...
1. MUST: MCP server registry implementations MUST use JavaScript Map instances for in-memory caching of server state, keyed by server identifier (URL for SSE/SHTTP transports, name for STDIO transport)

## Policy Block

- MUST MCP server registry implementations MUST use JavaScript Map instances for in-memory caching of server state, keyed by server identifier (URL for SSE/SHTTP transports, name for STDIO transport)
