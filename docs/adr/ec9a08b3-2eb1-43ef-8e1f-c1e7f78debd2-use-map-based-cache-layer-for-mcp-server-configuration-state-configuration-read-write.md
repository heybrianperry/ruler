...
1. MUST: Configuration read/write operations MUST use Map data structures keyed by server name (for stdio) or URL (for SSE/SHTTP) to cache and deduplicate server entries

## Policy Block

- MUST Configuration read/write operations MUST use Map data structures keyed by server name (for stdio) or URL (for SSE/SHTTP) to cache and deduplicate server entries
