...
1. MUST: FileSystemUtils path operations MUST reject and throw an error for any symlink target that resolves outside the designated workspace root boundary.

## Policy Block

- MUST FileSystemUtils path operations MUST reject and throw an error for any symlink target that resolves outside the designated workspace root boundary.
