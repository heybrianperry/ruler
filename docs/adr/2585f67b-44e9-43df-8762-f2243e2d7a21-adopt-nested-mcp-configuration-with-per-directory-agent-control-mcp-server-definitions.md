...
1. MUST: MCP server definitions MUST NOT propagate across directory boundaries; each directory's MCP configuration MUST contain only servers defined in its local ruler.toml

## Policy Block

- MUST MCP server definitions MUST NOT propagate across directory boundaries; each directory's MCP configuration MUST contain only servers defined in its local ruler.toml
