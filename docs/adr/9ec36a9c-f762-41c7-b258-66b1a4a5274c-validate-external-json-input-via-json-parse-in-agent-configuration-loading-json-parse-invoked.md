...
1. MUST: JSON.parse MUST be invoked on the raw string content returned from file system read operations, not on pre-processed or partially parsed data

## Policy Block

- MUST JSON.parse MUST be invoked on the raw string content returned from file system read operations, not on pre-processed or partially parsed data
