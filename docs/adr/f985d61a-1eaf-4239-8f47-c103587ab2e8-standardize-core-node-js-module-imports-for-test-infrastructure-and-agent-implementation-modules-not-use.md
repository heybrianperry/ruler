...
1. MUST_NOT: Modules MUST NOT use synchronous file system operations (fs.readFileSync, fs.writeFileSync) in production agent code where async alternatives exist.

## Policy Block

- MUST_NOT Modules MUST NOT use synchronous file system operations (fs.readFileSync, fs.writeFileSync) in production agent code where async alternatives exist.
