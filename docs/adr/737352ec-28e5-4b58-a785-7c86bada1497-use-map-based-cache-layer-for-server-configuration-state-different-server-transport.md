...
1. SHOULD: Different server transport types (SSE, SHTTP, STDIO) SHOULD maintain separate Map instances to enforce type safety and prevent key collisions.

## Policy Block

- SHOULD Different server transport types (SSE, SHTTP, STDIO) SHOULD maintain separate Map instances to enforce type safety and prevent key collisions.
