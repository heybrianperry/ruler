# Adopt Async/Await Concurrency Model for External API Interactions: External Calls Use
...
1. MUST: All external API calls MUST use await keyword rather than .then()/.catch() chains for Promise resolution

## Policy Block

- MUST All external API calls MUST use await keyword rather than .then()/.catch() chains for Promise resolution
