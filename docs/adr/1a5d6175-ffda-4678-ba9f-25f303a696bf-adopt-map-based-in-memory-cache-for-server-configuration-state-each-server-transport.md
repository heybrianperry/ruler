...
1. MUST: Each server transport type (SSE, SHTTP, STDIO) MUST maintain a separate Map instance to prevent namespace collisions between different transport protocols

## Policy Block

- MUST Each server transport type (SSE, SHTTP, STDIO) MUST maintain a separate Map instance to prevent namespace collisions between different transport protocols
