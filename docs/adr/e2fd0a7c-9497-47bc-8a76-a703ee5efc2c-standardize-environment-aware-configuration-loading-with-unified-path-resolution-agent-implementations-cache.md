# Standardize Environment-Aware Configuration Loading with Unified Path Resolution: Agent Implementations Cache
...
1. MAY: Agent implementations MAY cache loaded configuration for performance, but MUST provide mechanisms to reload configuration when environment changes

## Policy Block

- MAY Agent implementations MAY cache loaded configuration for performance, but MUST provide mechanisms to reload configuration when environment changes
