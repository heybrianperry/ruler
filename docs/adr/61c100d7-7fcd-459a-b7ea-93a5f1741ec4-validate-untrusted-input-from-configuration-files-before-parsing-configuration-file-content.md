...
1. MUST: All configuration file content read from disk MUST be validated against a defined schema before parsing with JSON.parse() or parseTOML()

## Policy Block

- MUST All configuration file content read from disk MUST be validated against a defined schema before parsing with JSON.parse() or parseTOML()
