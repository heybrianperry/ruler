...
1. MUST: MCP server configuration state MUST be cached in-memory using Map data structures keyed by server name (for stdio servers) or URL (for SSE/HTTP servers)

## Policy Block

- MUST MCP server configuration state MUST be cached in-memory using Map data structures keyed by server name (for stdio servers) or URL (for SSE/HTTP servers)
