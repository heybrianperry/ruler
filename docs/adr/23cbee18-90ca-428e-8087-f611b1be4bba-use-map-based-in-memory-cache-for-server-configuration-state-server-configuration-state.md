...
1. MUST: Server configuration state MUST be cached in Map instances during read-modify-write operations, keyed by server name for STDIO servers and by URL for SSE/SHTTP servers

## Policy Block

- MUST Server configuration state MUST be cached in Map instances during read-modify-write operations, keyed by server name for STDIO servers and by URL for SSE/SHTTP servers
