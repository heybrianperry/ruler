...
1. MUST: Server configuration state MUST be cached in-memory using Map data structures indexed by server name or URL during read-modify-write operations

## Policy Block

- MUST Server configuration state MUST be cached in-memory using Map data structures indexed by server name or URL during read-modify-write operations
