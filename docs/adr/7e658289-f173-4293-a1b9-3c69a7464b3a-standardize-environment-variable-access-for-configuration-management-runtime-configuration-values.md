# Standardize Environment Variable Access for Configuration Management: Runtime Configuration Values
...
1. MUST: All runtime configuration values that vary by environment (API keys, endpoints, feature flags, paths) MUST be accessed via environment variables using process.env

## Policy Block

- MUST All runtime configuration values that vary by environment (API keys, endpoints, feature flags, paths) MUST be accessed via environment variables using process.env
