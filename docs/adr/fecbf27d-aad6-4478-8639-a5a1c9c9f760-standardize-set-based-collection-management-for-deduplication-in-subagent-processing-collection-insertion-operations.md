...
1. MUST: Collection insertion operations MUST use the .add() method pattern (e.g., 'targets.add(target)', 'mapped.add(alias)') to maintain Set semantics

## Policy Block

- MUST Collection insertion operations MUST use the .add() method pattern (e.g., 'targets.add(target)', 'mapped.add(alias)') to maintain Set semantics
