...
1. MUST: Cache operations MUST use Map.set(key, value) to store or update server entries where key is derived from server.name or server URL properties.

## Policy Block

- MUST Cache operations MUST use Map.set(key, value) to store or update server entries where key is derived from server.name or server URL properties.
