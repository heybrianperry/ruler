# Standardize Environment-Aware Configuration Loading with Unified Path Resolution: Mcp Path Propagation
...
1. MUST: MCP path propagation MUST use standardized path resolution utilities (mcp.ts) to ensure consistent server discovery across agent types

## Policy Block

- MUST MCP path propagation MUST use standardized path resolution utilities (mcp.ts) to ensure consistent server discovery across agent types
