# Adopt TypeScript with Jest/Vitest for Unit Testing Framework: Each Module Class
...
1. MUST: Each module or class MUST have a corresponding test file with the same base name followed by .test.ts (e.g., AmpAgent.ts → AmpAgent.test.ts).

## Policy Block

- MUST Each module or class MUST have a corresponding test file with the same base name followed by .test.ts (e.g., AmpAgent.ts → AmpAgent.test.ts).
