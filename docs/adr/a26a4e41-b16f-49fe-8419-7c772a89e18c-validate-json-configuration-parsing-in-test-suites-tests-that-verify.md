...
1. MUST: Tests that verify agent configuration outputs MUST validate the presence of expected JSON keys (mcpServers, servers, contextFileName, $schema) after successful parsing

## Policy Block

- MUST Tests that verify agent configuration outputs MUST validate the presence of expected JSON keys (mcpServers, servers, contextFileName, $schema) after successful parsing
