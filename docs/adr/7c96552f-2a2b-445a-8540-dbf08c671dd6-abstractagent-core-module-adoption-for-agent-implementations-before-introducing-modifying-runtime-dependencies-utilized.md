...
1. MUST: Before introducing or modifying runtime dependencies utilized by agent implementations, the consumer MUST inspect the repository lock or resolution artifact to determine and verify the exact resolved version against dependency manifests.

## Policy Block

- MUST Before introducing or modifying runtime dependencies utilized by agent implementations, the consumer MUST inspect the repository lock or resolution artifact to determine and verify the exact resolved version against dependency manifests.
