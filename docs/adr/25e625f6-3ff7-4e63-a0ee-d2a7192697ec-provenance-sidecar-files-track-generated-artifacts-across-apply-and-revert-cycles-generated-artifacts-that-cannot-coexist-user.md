...
1. SHOULD: Generated artifacts that cannot coexist with user-created equivalents (i.e., artifacts in ruler-exclusive paths with no collision risk) SHOULD also receive provenance sidecars to maintain a uniform ownership signal across the codebase.

## Policy Block

- SHOULD Generated artifacts that cannot coexist with user-created equivalents (i.e., artifacts in ruler-exclusive paths with no collision risk) SHOULD also receive provenance sidecars to maintain a uniform ownership signal across the codebase.
