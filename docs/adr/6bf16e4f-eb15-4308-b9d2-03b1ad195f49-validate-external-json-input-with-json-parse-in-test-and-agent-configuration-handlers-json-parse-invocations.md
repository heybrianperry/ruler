...
1. MUST: JSON.parse invocations MUST operate on the raw string content returned from filesystem read operations, not on pre-processed or partially validated data

## Policy Block

- MUST JSON.parse invocations MUST operate on the raw string content returned from filesystem read operations, not on pre-processed or partially validated data
