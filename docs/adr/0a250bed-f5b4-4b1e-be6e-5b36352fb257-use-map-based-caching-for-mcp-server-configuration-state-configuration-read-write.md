...
1. SHOULD: Configuration read/write functions (readVSCodeSettings, writeVSCodeSettings, propagateMcpToOpenHands) SHOULD rebuild cache state from authoritative sources on each invocation

## Policy Block

- SHOULD Configuration read/write functions (readVSCodeSettings, writeVSCodeSettings, propagateMcpToOpenHands) SHOULD rebuild cache state from authoritative sources on each invocation
