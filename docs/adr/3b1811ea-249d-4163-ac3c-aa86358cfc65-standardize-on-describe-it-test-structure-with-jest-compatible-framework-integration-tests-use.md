...
1. SHOULD: Integration tests SHOULD use descriptive suite names that reflect the complete workflow being tested (e.g., 'ruler init → setup → apply → verify')

## Policy Block

- SHOULD Integration tests SHOULD use descriptive suite names that reflect the complete workflow being tested (e.g., 'ruler init → setup → apply → verify')
