...
1. MUST: Server configuration state MUST be cached in Map instances keyed by unique server identifiers (name for STDIO servers, url for SSE/SHTTP servers).

## Policy Block

- MUST Server configuration state MUST be cached in Map instances keyed by unique server identifiers (name for STDIO servers, url for SSE/SHTTP servers).
