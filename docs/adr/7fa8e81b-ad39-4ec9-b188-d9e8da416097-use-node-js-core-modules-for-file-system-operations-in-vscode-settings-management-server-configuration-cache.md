...
1. MUST: Server configuration cache MUST use Map data structures with server.name as keys and server objects as values (existingServerMap.set pattern)

## Policy Block

- MUST Server configuration cache MUST use Map data structures with server.name as keys and server objects as values (existingServerMap.set pattern)
