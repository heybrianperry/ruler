...
1. SHOULD: Cache instances SHOULD be scoped to the function performing the read-modify-write operation to avoid unintended state sharing across invocations

## Policy Block

- SHOULD Cache instances SHOULD be scoped to the function performing the read-modify-write operation to avoid unintended state sharing across invocations
