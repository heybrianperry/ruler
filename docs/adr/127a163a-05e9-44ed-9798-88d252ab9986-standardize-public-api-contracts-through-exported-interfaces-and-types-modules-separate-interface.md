...
1. SHOULD: Modules SHOULD separate interface definitions into dedicated files (e.g., IAgent.ts) when the interface is consumed by multiple implementations.

## Policy Block

- SHOULD Modules SHOULD separate interface definitions into dedicated files (e.g., IAgent.ts) when the interface is consumed by multiple implementations.
