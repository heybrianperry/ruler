...
1. SHOULD: Configuration parsing operations (parseTOML, JSON.parse) SHOULD be followed immediately by Array.find() lookups in test validation blocks to minimize scope of unsafe references

## Policy Block

- SHOULD Configuration parsing operations (parseTOML, JSON.parse) SHOULD be followed immediately by Array.find() lookups in test validation blocks to minimize scope of unsafe references
